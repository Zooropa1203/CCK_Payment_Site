...
router.get(
  ...
  (req, res) => {
    db.all(
      'SELECT id, email, name, phone, cck_id, role, created_at FROM users ORDER BY created_at DESC',
      (err, users) => {
        if (err) {
...
router.get('/profile', authenticateToken, (req, res) => {
  ...
  db.get(
    'SELECT id, email, name, phone, cck_id, role, created_at FROM users WHERE id = ?',
    [userId],
    (err, user) => {
      if (err) {
        return res.status(500).json({ message: 'Database error' });
...
router.put(
  ...
  (req, res) => {
    ...
    if (cck_id !== undefined) updates.cck_id = cck_id;
...
router.put(
  ...
  authenticateToken,
  requireRole(['administrator']),
  [body('role').isIn(['administrator', 'organizer', 'member'])],
  (req, res) => {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      return res.status(400).json({ errors: errors.array() });
