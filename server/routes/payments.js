...
router.post('/register', authenticateToken, [
  ...
  `, [competition_id, competition_id], (err, competition) => {
        if (err) {
        ...
        db.get('SELECT id FROM registrations WHERE user_id = ? AND competition_id = ?', [userId, competition_id], (err, existing) => {
            if (err) {
                return res.status(500).json({ message: 'Database error' });
            }
            if (existing) {
            ...
            const status = competition.current_participants >= competition.max_participants
                ? 'waitlist'
                : 'pending';
            // Create registration
            db.run(`
    ...
    try {
      ...
      `, [paymentKey, orderId], function (err) {
                if (err) {
                    ...
                    return res
                        .status(500)
                        .json({ message: 'Failed to update payment' });
                }
...
router.delete('/registration/:id', authenticateToken, (req, res) => {
    ...
    db.get('SELECT * FROM registrations WHERE id = ? AND user_id = ?', [id, userId], (err, registration) => {
        if (err) {
            return res.status(500).json({ message: 'Database error' });
        }
        if (!registration) {
        ...
        if (registration.status === 'confirmed' &&
            ...
            return res.status(400).json({
                message: 'Cannot cancel confirmed registration. Please contact support for refunds.',
            });
        }
        // Cancel registration
        db.run('UPDATE registrations SET status = "cancelled" WHERE id = ?', [id], function (err) {
            if (err) {
                ...
                return res
                    .status(500)
                    .json({ message: 'Failed to cancel registration' });
            }
