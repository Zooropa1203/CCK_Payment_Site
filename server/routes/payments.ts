  ...
  `,
      ...
      (err, competition: any) => {
        ...
        db.get(
          ...
          VALUES (?, ?, ?, ?)
        `,
              [userId, competition_id, JSON.stringify(events), status],
              function (err) {
                if (err) {
              ...
              VALUES (?, ?, 'KRW')
            `,
                    [this.lastID, competition.entry_fee],
                    function (err) {
                      if (err) {
    ...
    (err, registration: any) => {
      ...
      if (
        ...
        return res.status(400).json({
          message:
            'Cannot cancel confirmed registration. Please contact support for refunds.',
        });
      }
